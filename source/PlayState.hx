...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff2:Array<String> = [
		'Clear',
		'SDCB',
		'FC',
		'GFC',
	...
	override public function create()
	{
		...
		if(curStage == 'philly' && !ClientPrefs.optimization) {
		...
		if(!modchartSprites.exists('blammedLightsBlack') && !ClientPrefs.optimization) { //Creates blammed light black fade in case you didn't make your own
		...
		if(curStage == 'philly' && !ClientPrefs.optimization) insert(members.indexOf(blammedLightsBlack) + 1, phillyCityLightsEvent);
		if(!ClientPrefs.optimization) blammedLightsBlack = modchartSprites.get('blammedLightsBlack');
		if(!ClientPrefs.optimization) blammedLightsBlack.alpha = 0.0;
		#end
		...
		switch(curStage)
		{
			case 'limo':
				...
				if(!ClientPrefs.optimization) insert(members.indexOf(gfGroup) - 1, fastCar);
		...
		botplayTxt = new FlxText(-140, 5, FlxG.width - 800, "Autoplaying", 32);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curStage == 'mall') {
					if(!ClientPrefs.lowQuality || !ClientPrefs.optimization)
						upperBoppers.dance(true);
	
					if(!ClientPrefs.optimization) bottomBoppers.dance(true);
					if(!ClientPrefs.optimization) santa.dance(true);
				}
				...
				if(songNotes[1] > -1) { //Real notes
					...
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							...
							var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, isPlayer);
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if(blammedLightsBlackTween != null && !ClientPrefs.optimization)
				blammedLightsBlackTween.active = false;
			if(phillyCityLightsEventTween != null && !ClientPrefs.optimization)
			...
			for (i in 0...chars.length) {
				if(chars[i].colorTween != null && !ClientPrefs.optimization) {
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case 'schoolEvil':
				if(!ClientPrefs.lowQuality && !ClientPrefs.optimization && bgGhouls.animation.curAnim.finished) {
					bgGhouls.visible = false;
				}
			case 'philly':
				if (trainMoving && !ClientPrefs.optimization)
				{
				...
				if(!ClientPrefs.optimization) phillyCityLights.members[curLight].alpha -= (Conductor.crochet / 1000) * FlxG.elapsed * 1.5;
			case 'limo':
				if(!ClientPrefs.lowQuality || !ClientPrefs.optimization) {
			...
			case 'mall':
				if(heyTimer > 0 && !ClientPrefs.optimization) {
					heyTimer -= elapsed;
					if(heyTimer <= 0) {
		...
		switch(eventName) {
			case 'Hey!':
				...
				if(value != 0) {
					...
					if(curStage == 'mall' && !ClientPrefs.optimization) {
			...
			if(!ClientPrefs.optimization) {
				...
				if(lightId > 0 && curLightEvent != lightId && !ClientPrefs.optimization) {
					...
					if(blammedLightsBlack.alpha != 0 && !ClientPrefs.optimization) {
						if(blammedLightsBlackTween != null) {
							blammedLightsBlackTween.cancel();
						}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				...
				if (ClientPrefs.hitSounds) FlxG.sound.play(Paths.sound('ChartingTick'));
			}
		}
	}
	...
	function updateTrainPos():Void
	{
		...
		if (startedMoving && !ClientPrefs.optimization)
		{
			phillyTrain.x -= 400;
	...
	{
		FlxG.sound.play(Paths.soundRandom('thunder_', 1, 2));
		if(!ClientPrefs.lowQuality || !ClientPrefs.optimization) halloweenBG.animation.play('halloweem bg lightning strike');
	...
	{
		if(!ClientPrefs.lowQuality && !ClientPrefs.optimization && ClientPrefs.violence && curStage == 'limo') {
	...
	{
		if(curStage == 'limo' && !ClientPrefs.optimization) {
			limoMetalPole.x = -500;
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'mall':
				...
				if(heyTimer <= 0 && !ClientPrefs.optimization) bottomBoppers.dance(true);
				if(!ClientPrefs.optimization) santa.dance(true);
			...
			case 'limo':
				...
				if (FlxG.random.bool(10) && fastCarCanDrive && !ClientPrefs.optimization)
					fastCarDrive();
			case "philly":
				if (!trainMoving && !ClientPrefs.optimization)
					trainCooldown += 1;

				if (curBeat % 4 == 0 && !ClientPrefs.optimization)
				{
				...
				if (curBeat % 8 == 4 && !ClientPrefs.optimization && FlxG.random.bool(30) && !trainMoving && trainCooldown > 8)
				{
		...
		if (curStage == 'spooky' && !ClientPrefs.optimization && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
		{
			lightningStrikeShit();
		}
		lastBeatHit = curBeat;
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			*/
			...
			ratingString2 = ratingStuff2[0]; // setting this to smth just in case the script below does nothing
			if (songMisses == 0 && goods == 0 && bads == 0 && shits == 0) {
				...
				ratingString2 = ratingStuff2[3];
				
			} else if (songMisses == 0) {
			
				ratingString2 = ratingStuff2[2];
				
			} else if (songMisses < 10) {
			
				ratingString2 = ratingStuff2[1];
